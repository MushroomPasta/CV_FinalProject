...
class MobileNet(object):
    def __init__(self, lr, num_classes=1000, is_training=True,
        ...
        self.X = tf.placeholder(dtype=tf.int32, shape=[None,227,227,3])
        self.y = tf.placeholder(dtype=tf.int32, shape=[None,20])
        # construct model
        with tf.variable_scope(scope):
            # conv1
            net = conv2d(self.X, "conv_1", round(32 * width_multiplier), filter_size=3,
    ...
    def train(self, batch_generator, epoch, save_path, save_every_n, log_every_n):
        ...
        with self.session as sess:
            ...
            for i in range(epoch):
                ...
                for X_batch, y_batch in batch_generator:
                    loss_val, _ = sess.run([self.loss, self.train_op], feed_dict={self.X: X_batch, self.y: y_batch})
                end = time.time()
                if i % log_every_n == 0:
                    print('step: {}/{}... '.format(i+1, epoch),
                              'loss: {:.4f}... '.format(loss_val),
                              '{:.4f} sec/epoch'.format((end - start)))
                if (i % save_every_n == 0):
                    self.saver.save(sess, os.path.join(save_path, 'model'), global_step=i+1)
